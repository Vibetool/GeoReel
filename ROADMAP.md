...
- [x] travel 自动定速 — 已实现：省略 duration 则按路程÷groundSpeed(默认15000m/s，~1000km≈65s)自动规划时长，关键帧数随之自适应；不同长度路线自动统一舒适速度
- [x] 每帧瓦片 settle — 已实现：等瓦片完全加载并稳定 SETTLE_MS(默认500ms)再截图，杜绝"地图一直刷新"
- [x] labelScale 标签放大 — 已实现：城市/山脉名可放大 2~3 倍；防碰撞间距、边缘对齐随字号自适应，大字不重叠不出框
