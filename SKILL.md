...
> ⚠️ **Gotcha — scale `targetHeight`/`alt` by the exaggeration.** The terrain (and every POI anchor) renders at `realElevation × verticalExaggeration`, but a shot's `targetHeight` is fed to the camera-aim math *as written*. So with `verticalExaggeration: 2.0` and a subject at ~500 m real elevation, you must set `targetHeight: 1000` (500 × 2), not 500 — otherwise the camera aims ~500 m *below* the exaggerated ridge and the subject floats to the top of the frame with a huge empty foreground. Same for a `travel` shot's `alt` (keep it a few hundred m above the exaggerated ridgetop). Rule of thumb: `targetHeight ≈ realElevation × verticalExaggeration`. Always `--preview` after changing exaggeration. (See `scenes/jinshanling-sunrise.json`: 500 m wall, exag 2.0 → `targetHeight: 1000`.)
